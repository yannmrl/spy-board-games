...
function ok(j) {
    ...
    if (jI && !players.includes(jI)) {
        ...
        if (roles[j-1] == 'civil') {
            pop = `
                <label style="text-align: center; padding: 5%;">Civil</label>
            ...
            p_words[j-1] = word;
            pop = `
                <label style="text-align: center; padding: 5%;">Tra&icirc;tre</label>
            ...
            p_words[j-1] = word;
            pop = `
                <label style="text-align: center; padding: 5%;">Ma&icirc;tre du jeu</label>
                <label style="text-align: center; padding: 5%; margin-bottom: 2%">`+word+`</label>
                <label class="okButton" onclick="displayNameCard(`+(j+1)+`)">Ok</label>
            `;
            $('#pop').append(pop); 
            ...
            if (Math.random() < 0.5) {
                p_words[j-1] = word;
                pop = `
                    <label style="text-align: center; padding: 5%;">Fauteur de troubles</label>
                    <label style="text-align: center; padding: 5%; margin-bottom: 2%">??????</label>
                    <label class="okButton" onclick="displayNameCard(`+(j+1)+`)">Ok</label>
                `;
            } else {
                pop = `
...
function timer() {
    ...
    minutes = parseInt(elem.innerHTML.split(":")[0]);
    seconds = parseInt(elem.innerHTML.split(":")[1]);
    if (seconds) {
        if (seconds <= 10) {
            elem.innerHTML = minutes+":0"+(seconds-1);            
            if (minutes == 0) {
                if (seconds == 11) {window.speechSynthesis.speak(new SpeechSynthesisUtterance('dix secondes restantes'));}
                else if (seconds == 1) {window.speechSynthesis.speak(new SpeechSynthesisUtterance('Le temps imparti est ecouler'))}
                else if (seconds <= 6) {window.speechSynthesis.speak(new SpeechSynthesisUtterance(String(seconds-1)));}
            }
        } else {
            elem.innerHTML = minutes+":"+(seconds-1);
            if (seconds == 31 && minutes == 0) {window.speechSynthesis.speak(new SpeechSynthesisUtterance('trente secondes restantes'));}
        }
    } else {
        if (minutes) {
            elem.innerHTML = (minutes-1)+":59";
            if (minutes == 1) {window.speechSynthesis.speak(new SpeechSynthesisUtterance('une minute restante'));}
        } else {
            stop_ = true;
        }
    }
...
function kill(him) {
    if (playing) {
        ...
        if (roles[him.id] == "traitre") {
            ...
            $('.options').remove();
            reveal();
        } else if (roles[him.id] == "fdt") {
            document.getElementById('log').innerHTML = him.innerHTML+' &eacute;tait le fauteur de troubles';
        } else {
...
function displayMainInfo() {
    ...
    $('body').append(`
        ...
        <div id="pop" class="pop" style="padding: 5%; overflow-y: scroll;">
            <label style="margin-bottom: 3%; font-weight: bolder;">Insider</label>
            <label class="desc">Un ma&icirc;tre du jeu, un tra&icirc;tre et des civils.</label>
            <label class="desc">Le but du tra&icirc;tre est de se fondre parmis les civils et des les aider à trouver un mot que seul lui et le ma&icirc;tre du jeu connaissent.</label>
            <label class="desc">Tout le monde peut poser des questions au ma&icirc;tre du jeu, auxquelles il ne peut r&eacute;pondre que par oui ou par non.</label>
            <label class="desc">Une fois que le mot est trouv&eacute;, il s'agit de d&eacute;masquer le tra&icirc;tre.</label>
            <label class="desc">On vote d'abord pour celui qui a trouv&eacute; le mot, puis, si on l'innocente, on cherche le tra&icirc;tre parmis les joueurs restants.</label>
        </div>
    `);
